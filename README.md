...
(100%); 10/11 rectified with a clean, tightly-cropped result including full
content (verified visually against the input side by side -- see
`head-to-head-comparison/`, generated by pasting each sample's raw image
next to its rectified output); 1 hard diagonal shot on a reflective steel
background where the cascade makes no change and honestly reports
`boundary_not_found`/`noop_quad` rather than guessing. Output image
resolution never exceeds the source's -- it's smaller than the input by
whatever fraction was background/margin being cropped away, which is the
correct, intended effect of rectification, not a quality loss (sharpness
was checked quantitatively too: perspective-correcting a sharp source photo
costs ~7.6% Laplacian-variance sharpness, the unavoidable resampling cost
any scanner pays; an already-blurry source photo comes out at ~0% additional
loss -- the pipeline doesn't amplify pre-existing camera blur).
...
- **Method B could confidently truncate real content** when one physical
  edge had genuinely low contrast against the background (measured on a
  real photo: paper-vs-table gray values drifted only ~15 levels over 80px
  at a receipt's rounded/notched top -- not enough for Canny to fire at any
  reasonable threshold). No line near the true edge ever entered the
  candidate pool, so the perimeter-strength scorer picked the
  best-available *wrong* line -- in this case a printed divider between two
  receipt sections -- and returned a "successful," high-confidence quad
  that silently cropped off the entire header. `_text_containment_ratio`
  (added for the bug above) measured this: 0.38 on the bad quad vs. >=0.88
  on every correctly-detected real quad measured. Re-added as a **hard**
  reject floor (`text_containment_floor`, 0.6) -- safe now because it only
  applies once text_mask has already passed the `min_text_blobs`
  reliability check, so it can't reject on the same noisy-mask cases that
  motivated softening it in the first place. Rejecting here correctly
  forced the cascade down to GrabCut, which segments by region/color
  rather than edge lines and doesn't need a sharp local gradient to find
  the true boundary -- it landed within a few pixels of the real top edge.
